...
<title>SmartGo — OOP Final Presentation</title>
<link href="https://fonts.googleapis.com/css2?family=Syne:wght@400;700;800&family=DM+Sans:wght@300;400;500;600;700&family=JetBrains+Mono:wght@400;700&display=swap" rel="stylesheet">
<style>
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
:root{
  --g1:#00c896;--g2:#0090ff;--g3:#a855f7;
  --dark2:#f8fafc;--card:#ffffff;--border:#e2e8f0;
  --muted:#64748b;--text:#0f172a;--code-bg:#0f172a;
}
body{font-family:"DM Sans",sans-serif;background:#f1f5f9;color:var(--text);padding:40px 20px;font-size:15px;}
@media print{body{padding:0;margin:0;}.slide-wrap{page-break-after:always;margin:0;padding:0;}.slide{box-shadow:none!important;border-radius:0!important;width:100%!important;height:100vh!important;}}
.slide-wrap{display:flex;justify-content:center;margin-bottom:48px;}
.slide{width:1280px;height:720px;border-radius:20px;overflow:hidden;position:relative;border:1px solid var(--border);box-shadow:0 30px 60px rgba(15,23,42,0.1);flex-shrink:0;background:var(--dark2);}
.bg{position:absolute;inset:0;z-index:0;}
.pad{position:relative;z-index:2;padding:44px 60px;height:100%;display:flex;flex-direction:column;gap:14px;}
.sn{position:absolute;bottom:28px;right:40px;font-size:12px;font-weight:700;color:rgba(15,23,42,0.3);font-family:"JetBrains Mono",monospace;letter-spacing:.1em;}
.label{font-family:"JetBrains Mono",monospace;font-weight:700;letter-spacing:.15em;text-transform:uppercase;font-size:10px;color:var(--g1);padding:4px 12px;background:rgba(0,200,150,.08);border:1px solid rgba(0,200,150,0.15);border-radius:20px;display:inline-block;width:fit-content;}
.heading{font-family:"Syne",sans-serif;font-weight:700;line-height:1.15;letter-spacing:-.02em;}
.display{font-family:"Syne",sans-serif;font-weight:800;line-height:1;letter-spacing:-.04em;}
.grad{background:linear-gradient(135deg,var(--g1) 0%,var(--g2) 50%,var(--g3) 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.code{background:var(--code-bg);border-radius:10px;padding:16px 18px;font-family:"JetBrains Mono",monospace;font-size:11px;color:#cbd5e1;border:1px solid rgba(0,0,0,0.1);line-height:1.55;overflow:hidden;}
.ck{color:#f472b6}.cv{color:#c084fc}.cs{color:#38bdf8}.cc{color:#64748b}.ct{color:#4ade80}.cm{color:#fb923c}.cy{color:#facc15}
.card{background:var(--card);border:1px solid var(--border);border-radius:14px;padding:20px 22px;box-shadow:0 4px 6px -1px rgba(0,0,0,0.05);}
.uml-box{background:#ffffff;border:1.5px solid #cbd5e1;border-radius:4px;min-width:150px;display:flex;flex-direction:column;font-size:10px;font-family:"JetBrains Mono",monospace;}
.uml-name{background:#f8fafc;padding:8px 12px;text-align:center;font-weight:700;font-size:11px;border-bottom:1.5px solid #cbd5e1;font-family:"Syne",sans-serif;color:var(--text);}
.uml-name.abstract{color:var(--g1);font-style:italic;}
.uml-name.iface{color:var(--g3);}
.uml-section{padding:8px 12px;border-bottom:1px solid #e2e8f0;min-height:26px;line-height:1.7;color:#475569;}
.uml-section:last-child{border-bottom:none;}
.vs-grid{display:grid;grid-template-columns:1fr 36px 1fr;gap:16px;align-items:center;}
.vs-bad{background:#fff1f2;border:1px solid #fecaca;border-radius:10px;padding:14px 16px;color:#991b1b;font-size:12.5px;line-height:1.5;}
.vs-good{background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:14px 16px;color:#166534;font-size:12.5px;line-height:1.5;}
.vs-badge{width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,var(--g1),var(--g2));display:flex;align-items:center;justify-content:center;font-weight:800;font-size:11px;color:#fff;font-family:"Syne",sans-serif;}
.bullet{display:flex;gap:10px;align-items:flex-start;margin-bottom:9px;font-size:14px;line-height:1.5;}
.bullet::before{content:"›";color:var(--g1);font-weight:900;font-size:18px;line-height:1.2;flex-shrink:0;}
.concept-tag{display:inline-block;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:700;font-family:"JetBrains Mono",monospace;letter-spacing:.05em;}
.tag-green{background:rgba(0,200,150,.1);color:var(--g1);border:1px solid rgba(0,200,150,.2);}
.tag-blue{background:rgba(0,144,255,.1);color:var(--g2);border:1px solid rgba(0,144,255,.2);}
.tag-purple{background:rgba(168,85,247,.1);color:var(--g3);border:1px solid rgba(168,85,247,.2);}
.tag-orange{background:rgba(251,146,60,.1);color:#ea580c;border:1px solid rgba(251,146,60,.2);}
.two-col{display:grid;grid-template-columns:1fr 1fr;gap:20px;flex:1;}
.arch-layer{background:var(--card);border:1px solid var(--border);border-radius:10px;padding:12px 18px;display:flex;align-items:center;gap:16px;}
.arch-label{font-family:"Syne",sans-serif;font-weight:700;font-size:13px;min-width:120px;}
.hl{background:linear-gradient(135deg,rgba(0,200,150,.04),rgba(0,144,255,.04));border:1px solid rgba(0,200,150,.1);border-radius:12px;padding:16px 20px;}
...
<div style="height:100%;display:flex;flex-direction:column;justify-content:center;padding:0 40px;gap:60px;">
  <div style="border-left:4px solid var(--g1);padding-left:32px;">
    ...
    <div style="font-size:24px;color:var(--text);font-weight:500;margin-top:10px;letter-spacing:-0.01em;opacity:0.8;">Advanced Travel Management Ecosystem</div>
    <div style="font-family:'JetBrains Mono',monospace;font-size:13px;color:var(--muted);margin-top:8px;text-transform:uppercase;letter-spacing:0.1em;">Object-Oriented Architecture • Java Implementation</div>
  </div>

  <div style="display:grid;grid-template-columns:1.5fr 1fr;gap:80px;align-items:start;">
    <div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--g1);font-weight:700;margin-bottom:24px;text-transform:uppercase;letter-spacing:0.2em;">Engineering Team · Group B2</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:32px 24px;">
        <div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:18px;color:var(--text);">Abdul Rehman</div><div style="font-size:12px;color:var(--muted);margin-top:4px;font-family:'JetBrains Mono',monospace;">ID: 576841</div></div>
        <div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:18px;color:var(--text);">M. Yousaf Khan</div><div style="font-size:12px;color:var(--muted);margin-top:4px;font-family:'JetBrains Mono',monospace;">ID: 572229</div></div>
        <div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:18px;color:var(--text);">Hamza Basit</div><div style="font-size:12px;color:var(--muted);margin-top:4px;font-family:'JetBrains Mono',monospace;">ID: 557844</div></div>
        <div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:18px;color:var(--text);">Rehan Ahmad</div><div style="font-size:12px;color:var(--muted);margin-top:4px;font-family:'JetBrains Mono',monospace;">ID: 553327</div></div>
      </div>
    </div>

    <div style="border-top:1px solid var(--border);padding-top:24px;display:flex;flex-direction:column;gap:24px;">
      <div style="display:flex;justify-content:space-between;align-items:baseline;border-bottom:1px solid rgba(0,0,0,0.03);padding-bottom:12px;">
        <span style="font-size:12px;color:var(--muted);font-family:'JetBrains Mono',monospace;text-transform:uppercase;">Project Domain</span>
        <span style="font-size:14px;font-weight:600;color:var(--text);">Travel & Logistics</span>
      </div>
      <div style="display:flex;justify-content:space-between;align-items:baseline;border-bottom:1px solid rgba(0,0,0,0.03);padding-bottom:12px;">
        <span style="font-size:12px;color:var(--muted);font-family:'JetBrains Mono',monospace;text-transform:uppercase;">Course</span>
        <span style="font-size:14px;font-weight:600;color:var(--text);">CS-2xx OOP</span>
      </div>
      <div style="display:flex;justify-content:space-between;align-items:baseline;border-bottom:1px solid rgba(0,0,0,0.03);padding-bottom:12px;">
        <span style="font-size:12px;color:var(--muted);font-family:'JetBrains Mono',monospace;text-transform:uppercase;">Stack</span>
        <span style="font-size:14px;font-weight:600;color:var(--text);">Java SE · Swing</span>
      </div>
    </div>
  </div>
</div>''', bg="var(--dark2)"))
...
<div style="display:grid;grid-template-columns:1.2fr 1fr;gap:40px;flex:1;">
  <div style="display:flex;flex-direction:column;gap:24px;">
    <div style="border-left:3px solid #ef4444;padding-left:20px;">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:18px;color:#ef4444;margin-bottom:12px;text-transform:uppercase;letter-spacing:0.05em;">The Problem</div>
      <div style="display:flex;flex-direction:column;gap:12px;font-size:14px;color:var(--muted);line-height:1.6;">
        <div>• <strong>Fragmented Systems:</strong> Travelers currently manage flights, hotels, and tours across 3+ disconnected applications.</div>
        <div>• <strong>Manual Calculation:</strong> Fee structures and platform commissions are often calculated manually, leading to financial errors.</div>
        <div>• <strong>Persistence Gaps:</strong> Critical user state and booking history are lost between sessions due to lack of local persistence.</div>
      </div>
    </div>
    
    <div style="border-left:3px solid var(--g1);padding-left:20px;background:rgba(0,200,150,0.03);padding-top:16px;padding-bottom:16px;border-radius:0 4px 4px 0;">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:18px;color:var(--g1);margin-bottom:12px;text-transform:uppercase;letter-spacing:0.05em;">The SmartGo Answer</div>
      <div style="font-size:15px;color:var(--text);line-height:1.6;">
        A <strong>Unified Java Core</strong> where every travel entity is a first-class Object. One codebase handles the entire lifecycle: Discovery → Booking → Billing → Review.
      </div>
    </div>
  </div>

  <div style="border:1px solid var(--border);padding:24px;background:#fff;">
    <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--muted);font-weight:700;margin-bottom:20px;text-transform:uppercase;letter-spacing:0.1em;">Feature Matrix</div>
    <div style="display:flex;flex-direction:column;gap:16px;">
      <div style="display:flex;gap:16px;align-items:baseline;">
        <div style="width:8px;height:8px;background:var(--g1);flex-shrink:0;"></div>
        <div><div style="font-weight:700;font-size:14px;">Flight Booking</div><div style="font-size:12px;color:var(--muted);">Global search and instant seat reservation.</div></div>
      </div>
      <div style="display:flex;gap:16px;align-items:baseline;">
        <div style="width:8px;height:8px;background:var(--g2);flex-shrink:0;"></div>
        <div><div style="font-weight:700;font-size:14px;">Hotel Reservations</div><div style="font-size:12px;color:var(--muted);">Rating-based discovery with guest management.</div></div>
      </div>
      <div style="display:flex;gap:16px;align-items:baseline;">
        <div style="width:8px;height:8px;background:var(--g3);flex-shrink:0;"></div>
        <div><div style="font-weight:700;font-size:14px;">Tour Packages</div><div style="font-size:12px;color:var(--muted);">Multi-day itineraries with optional meal plans.</div></div>
      </div>
      <div style="display:flex;gap:16px;align-items:baseline;">
        <div style="width:8px;height:8px;background:#f59e0b;flex-shrink:0;"></div>
        <div><div style="font-weight:700;font-size:14px;">Automated Billing</div><div style="font-size:12px;color:var(--muted);">5% platform fee computation and persistence.</div></div>
      </div>
      <div style="display:flex;gap:16px;align-items:baseline;">
        <div style="width:8px;height:8px;background:#fb923c;flex-shrink:0;"></div>
        <div><div style="font-weight:700;font-size:14px;">Review Architecture</div><div style="font-size:12px;color:var(--muted);">Standardized feedback loop across all entities.</div></div>
      </div>
    </div>
  </div>
</div>'''))
...
<div style="display:grid;grid-template-columns:1fr 1.1fr;gap:40px;flex:1;">
  <div style="display:flex;flex-direction:column;gap:16px;">
    <div style="border-left:3px solid var(--g3);padding:14px 20px;background:#fff;border:1px solid var(--border);border-left-width:3px;">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g3);text-transform:uppercase;letter-spacing:0.05em;margin-bottom:6px;">UI Layer</div>
      <div style="font-size:12px;color:var(--muted);">Main.java • Java Swing UI • Output Redirection via ByteArrayOutputStream</div>
    </div>
    <div style="text-align:center;color:var(--border);font-size:12px;">▼</div>
    <div style="border-left:3px solid var(--g2);padding:14px 20px;background:#fff;border:1px solid var(--border);border-left-width:3px;">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g2);text-transform:uppercase;letter-spacing:0.05em;margin-bottom:6px;">Service Layer</div>
      <div style="font-size:12px;color:var(--muted);">Stateless Business Logic • AuthService • BookingService • ReviewService</div>
    </div>
    <div style="text-align:center;color:var(--border);font-size:12px;">▼</div>
    <div style="border-left:3px solid var(--g1);padding:14px 20px;background:#fff;border:1px solid var(--border);border-left-width:3px;">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g1);text-transform:uppercase;letter-spacing:0.05em;margin-bottom:6px;">Model Layer</div>
      <div style="font-size:12px;color:var(--muted);">POJOs • Inheritance Hierarchies (Person, Transport) • Domain Entities</div>
    </div>
    <div style="text-align:center;color:var(--border);font-size:12px;">▼</div>
    <div style="border-left:3px solid #f59e0b;padding:14px 20px;background:#fff;border:1px solid var(--border);border-left-width:3px;">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:#f59e0b;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:6px;">Data Layer</div>
      <div style="font-size:12px;color:var(--muted);">DataStore DAO • Pipe-Delimited Flat Files (.txt) • Buffered Persistence</div>
    </div>
  </div>

  <div style="display:flex;flex-direction:column;gap:24px;">
    <div style="border:1px solid var(--border);padding:20px;background:#fff;">
      <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--muted);font-weight:700;margin-bottom:16px;text-transform:uppercase;letter-spacing:0.1em;">Key Design Decisions</div>
      <div style="display:flex;flex-direction:column;gap:14px;font-size:13px;line-height:1.5;">
        <div><strong>• Flat-File Persistence:</strong> Used for lightweight state management without the overhead of a SQL database.</div>
        <div><strong>• Service Locator Pattern:</strong> Static service classes act as lightweight alternatives to full dependency injection containers.</div>
        <div><strong>• Centralized DAO:</strong> DataStore manages all I/O, ensuring consistent parsing and serialization across all 16 domain models.</div>
      </div>
    </div>

    <div style="border:1px solid var(--border);padding:20px;background:#fff;flex:1;">
      <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--muted);font-weight:700;margin-bottom:16px;text-transform:uppercase;letter-spacing:0.1em;">Package Structure</div>
      <pre style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--text);line-height:1.4;">
src/
 ├── models/     <span style="color:var(--muted);">// 16 Domain Classes</span>
 ├── services/   <span style="color:var(--muted);">// 6 Business Logic Classes</span>
 ├── data/       <span style="color:var(--muted);">// DataStore + .txt Files</span>
 ├── enums/      <span style="color:var(--muted);">// Type-Safe Constants</span>
 ├── exceptions/ <span style="color:var(--muted);">// Custom SmartGo Errors</span>
 └── Main.java   <span style="color:var(--muted);">// Swing UI & Entry Point</span>
      </pre>
    </div>
  </div>
</div>'''))
...
<div class="two-col">
  <div style="display:flex;align-items:center;justify-content:center;flex-direction:column;gap:10px;">
    <div class="uml-box" style="width:250px;">
      <div class="uml-name abstract">«abstract» Person</div>
      <div class="uml-section">- id : int<br>- name : String<br>- email : String<br>- phone : String<br>- passwordHash : String<br>- createdAt : String</div>
      <div class="uml-section">+ getId() : int  + getName() : String<br>«abstract» + getRole() : String</div>
    </div>
    <div style="display:flex;gap:60px;align-items:flex-start;">
      <div style="display:flex;flex-direction:column;align-items:center;">
        <div style="width:2px;height:14px;background:#334155;"></div>
        <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-bottom:14px solid #334155;"></div>
        <div class="uml-box" style="width:185px;"><div class="uml-name">User</div><div class="uml-section">- address : String<br>- lastLogin : String</div><div class="uml-section">+ getRole() : String<br>+ getAddress() : String</div></div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:center;">
        <div style="width:2px;height:14px;background:#334155;"></div>
        <div style="width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-bottom:14px solid #334155;"></div>
        <div class="uml-box" style="width:185px;"><div class="uml-name">Admin</div><div class="uml-section">- role : String<br>- lastLogin : String</div><div class="uml-section">+ getRole() : String<br>+ getAdminRole() : String</div></div>
      </div>
    </div>
    <div style="font-size:11px;color:var(--muted);font-family:JetBrains Mono,monospace;">▷ hollow triangle = extends (Inheritance)</div>
  </div>
  <div style="display:flex;flex-direction:column;gap:12px;justify-content:center;">
    <div class="hl">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g1);margin-bottom:8px;">What this module shows</div>
      <div class="bullet">Person is <strong>abstract</strong> — you can never instantiate a plain Person object. Enforced at compile time.</div>
      <div class="bullet"><strong>getRole()</strong> is declared abstract, forcing both User and Admin to provide their own implementation.</div>
      <div class="bullet"><strong>passwordHash</strong> is in Person — security-sensitive fields are in one protected location.</div>
    </div>
    <div class="vs-grid">
      <div class="vs-bad"><strong>❌ Without:</strong> User and Admin duplicate name/email/phone/password fields — zero shared behavior.</div>
...
<div class="two-col">
  <div style="display:flex;flex-direction:column;align-items:center;gap:8px;">
    <div class="uml-box" style="width:255px;">
      <div class="uml-name abstract">«abstract» Transport</div>
      <div class="uml-section">- id : int  - destinationId : int<br>- type : String  - price : double  - capacity : int</div>
      <div class="uml-section">+ getPrice() : double  + setCapacity() : void<br>«abstract» + getScheduleSummary() : String</div>
    </div>
    <div style="display:flex;gap:20px;align-items:flex-start;justify-content:center;">
      <div style="display:flex;flex-direction:column;align-items:center;">
        <div style="width:2px;height:10px;background:#334155;"></div><div style="width:0;height:0;border-left:7px solid transparent;border-right:7px solid transparent;border-bottom:12px solid #334155;"></div>
        <div class="uml-box" style="width:150px;"><div class="uml-name">Flight</div><div class="uml-section">- airline : String<br>- flightNumber : String<br>- flightClass : String<br>- departureTime : String<br>- returnTime : String</div><div class="uml-section">+ getScheduleSummary()<br>+ getDisplayName()<br>+ getReviewType()</div></div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:center;">
        <div style="width:2px;height:10px;background:#334155;"></div><div style="width:0;height:0;border-left:7px solid transparent;border-right:7px solid transparent;border-bottom:12px solid #334155;"></div>
        <div class="uml-box" style="width:138px;"><div class="uml-name">Bus</div><div class="uml-section">- operator : String<br>- busType : String<br>- departureTime : String<br>- arrivalTime : String</div><div class="uml-section">+ getScheduleSummary()</div></div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:center;">
        <div style="width:2px;height:10px;background:#334155;"></div><div style="width:0;height:0;border-left:7px solid transparent;border-right:7px solid transparent;border-bottom:12px solid #334155;"></div>
        <div class="uml-box" style="width:138px;"><div class="uml-name">Train</div><div class="uml-section">- operator : String<br>- trainNumber : String<br>- departureTime : String<br>- arrivalTime : String</div><div class="uml-section">+ getScheduleSummary()</div></div>
      </div>
    </div>
  </div>
  <div style="display:flex;flex-direction:column;gap:12px;justify-content:center;">
    <div class="hl">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g2);margin-bottom:8px;">What this module shows</div>
      <div class="bullet">Transport is abstract. <strong>getScheduleSummary()</strong> is the abstract method — Flight returns departure+return, Bus/Train return departure+arrival.</div>
      <div class="bullet">Flight additionally implements <strong>Reviewable</strong>, making it the only transport users can review.</div>
      <div class="bullet">Price and capacity live in Transport once — no duplication across 3 subclasses.</div>
    </div>
    <div class="vs-grid">
      <div class="vs-bad"><strong>❌ Without:</strong> Three separate classes each repeat price, capacity, and ID logic — 3× the bugs.</div>
      <div class="vs-badge">VS</div>
...
<div class="two-col">
  <div style="display:flex;flex-direction:column;align-items:center;gap:12px;justify-content:center;">
    <div class="uml-box" style="width:220px;"><div class="uml-name iface">«interface» Reviewable</div><div class="uml-section">+ getId() : int<br>+ getDisplayName() : String<br>+ getReviewType() : String</div></div>
    <div style="display:flex;gap:40px;justify-content:center;align-items:flex-start;width:100%;">
      <div style="display:flex;flex-direction:column;align-items:center;">
        <div style="width:2px;height:12px;border-left:2px dashed #a855f7;"></div><div style="width:0;height:0;border-left:7px solid transparent;border-right:7px solid transparent;border-bottom:12px solid #a855f7;"></div>
        <div class="uml-box" style="width:175px;"><div class="uml-name">Flight</div><div class="uml-section">extends Transport<br>implements Reviewable</div><div class="uml-section">+ getDisplayName()<br>+ getReviewType()</div></div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:center;">
        <div style="width:2px;height:12px;border-left:2px dashed #a855f7;"></div><div style="width:0;height:0;border-left:7px solid transparent;border-right:7px solid transparent;border-bottom:12px solid #a855f7;"></div>
        <div class="uml-box" style="width:185px;"><div class="uml-name">Hotel</div><div class="uml-section">- id : int  - name : String<br>- rating : int  - pricePerNight : double</div><div class="uml-section">implements Reviewable<br>+ getDisplayName()  + getReviewType()</div></div>
      </div>
    </div>
    <div style="font-size:10.5px;color:#a855f7;font-family:JetBrains Mono,monospace;">⋯▷ dashed triangle = implements (Interface)</div>
  </div>
  <div style="display:flex;flex-direction:column;gap:12px;justify-content:center;">
    <div class="hl">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g3);margin-bottom:8px;">What this module shows</div>
      <div class="bullet">Reviewable is a <strong>contract</strong> — any class implementing it promises getId(), getDisplayName(), getReviewType().</div>
      <div class="bullet">ReviewService can accept <strong>any Reviewable</strong> — Hotel, Flight, or any future type — no instanceof checks.</div>
      <div class="bullet">Flight uses <strong>multiple contracts</strong>: extends Transport AND implements Reviewable — Java's safe near-multiple-inheritance.</div>
    </div>
...
<div class="two-col">
  <div style="display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap;">
    <div class="uml-box" style="width:168px;"><div class="uml-name">Booking</div><div class="uml-section">- id : int<br>- userId : int<br>- bookingType : String<br>- transportId : int<br>- bookedAt : String<br>- status : BookingStatus</div><div class="uml-section">+ getStatus()  + setStatus()</div></div>
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;"><div style="font-size:9px;color:#0ea5e9;font-family:JetBrains Mono,monospace;">1</div><div style="width:22px;height:2px;background:#0ea5e9;"></div><div style="width:0;height:0;border-top:5px solid transparent;border-bottom:5px solid transparent;border-left:10px solid #0ea5e9;"></div><div style="font-size:9px;color:#0ea5e9;font-family:JetBrains Mono,monospace;">1</div></div>
    <div class="uml-box" style="width:168px;"><div class="uml-name">Bill</div><div class="uml-section">- id : int<br>- bookingId : int<br>- baseAmount : double<br>- platformFee : double<br>- totalAmount : double<br>- status : String</div><div class="uml-section">+ getTotalAmount()  + setStatus()</div></div>
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;"><div style="font-size:9px;color:#0ea5e9;font-family:JetBrains Mono,monospace;">1</div><div style="width:22px;height:2px;background:#0ea5e9;"></div><div style="width:0;height:0;border-top:5px solid transparent;border-bottom:5px solid transparent;border-left:10px solid #0ea5e9;"></div><div style="font-size:9px;color:#0ea5e9;font-family:JetBrains Mono,monospace;">1</div></div>
    <div class="uml-box" style="width:158px;"><div class="uml-name">Payment</div><div class="uml-section">- id : int<br>- billId : int<br>- amount : double<br>- method : String<br>- paidAt : String<br>- status : String</div><div class="uml-section">+ getMethod()  + setStatus()</div></div>
  </div>
  <div style="display:flex;flex-direction:column;gap:12px;justify-content:center;">
    <div class="hl">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g2);margin-bottom:8px;">Composition Chain</div>
      <div class="bullet"><strong>Booking</strong> knows nothing about money. It delegates to Bill.</div>
      <div class="bullet"><strong>Bill</strong> auto-computes total = baseAmount + platformFee in its constructor — no external calculation needed.</div>
      <div class="bullet"><strong>Payment</strong> records the method (CASH / CARD / BANK_TRANSFER) from the PaymentMethod enum.</div>
      <div class="bullet">Each step is a <strong>separate concern</strong> — textbook Single Responsibility Principle.</div>
    </div>
...
<div class="two-col">
  <div style="display:flex;flex-direction:column;gap:10px;justify-content:center;">
    <div style="display:flex;gap:10px;align-items:center;">
      <div class="uml-box" style="width:165px;"><div class="uml-name">Destination</div><div class="uml-section">- id : int<br>- city : String<br>- country : String<br>- attractions : String</div><div class="uml-section">+ getCity() : String</div></div>
      <div style="width:18px;height:2px;background:#0ea5e9;flex-shrink:0;"></div><div style="width:0;height:0;border-top:5px solid transparent;border-bottom:5px solid transparent;border-left:10px solid #0ea5e9;flex-shrink:0;"></div>
      <div class="uml-box" style="width:175px;"><div class="uml-name">TourPlan</div><div class="uml-section">- id : int<br>- destinationId : int  - adminId : int<br>- title : String  - durationDays : int<br>- basePrice : double  - status : String</div><div class="uml-section">+ setStatus()  + setBasePrice()</div></div>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:4px;">
      <div class="uml-box" style="width:148px;"><div class="uml-name">MealPlan</div><div class="uml-section">- tourPlanId : int<br>- name : String  - price : double</div><div class="uml-section">+ getPrice() : double</div></div>
      <div class="uml-box" style="width:148px;"><div class="uml-name">TourPlanHotel</div><div class="uml-section">- tourPlanId : int<br>- hotelId : int<br>- isDefault : boolean</div><div class="uml-section">+ isDefault() : boolean</div></div>
      <div class="uml-box" style="width:152px;"><div class="uml-name">TourPlanTransport</div><div class="uml-section">- tourPlanId : int<br>- transportId : int</div><div class="uml-section">+ getTourPlanId()</div></div>
    </div>
  </div>
  <div style="display:flex;flex-direction:column;gap:12px;justify-content:center;">
    <div class="hl">
      <div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g1);margin-bottom:8px;">Relationship Types Used</div>
      <div class="bullet"><strong style="color:#0ea5e9;">Association (→)</strong>: TourPlan references Destination via destinationId — loosely coupled.</div>
      <div class="bullet"><strong style="color:var(--g1);">Composition</strong>: MealPlan cannot exist without its TourPlan — tourPlanId FK, always loaded together.</div>
      <div class="bullet"><strong style="color:var(--g3);">Link tables</strong>: TourPlanHotel and TourPlanTransport model many-to-many relationships.</div>
    </div>
...
<div class="ph" style="flex:1;border-radius:12px;">
  <!-- REPLACE this entire div with:
       <img src="your_erd.png" style="max-width:100%;max-height:100%;object-fit:contain;border-radius:12px;"/> -->
...
<div class="two-col">
  <div style="display:flex;flex-direction:column;gap:10px;">
    <div class="code"><span class="cc">// LSP — Transport reference holds any subtype</span><br><span class="cc">// without ANY change in calling code behavior</span><br><br><span class="ck">public static</span> <span class="cv">Booking</span> <span class="cm">bookFlight</span>(<span class="cs">int</span> userId, <span class="cs">int</span> flightId) {<br><br>&nbsp;&nbsp;<span class="cc">// Flight IS-A Transport — fully substitutable</span><br>&nbsp;&nbsp;<span class="cv">Flight</span> flight = FlightService.<span class="cm">getFlightById</span>(flightId);<br><br>&nbsp;&nbsp;<span class="cc">// getPrice() from Transport — works on ANY subtype</span><br>&nbsp;&nbsp;<span class="ck">double</span> fee = flight.<span class="cm">getPrice</span>() * PLATFORM_FEE_RATE;<br><br>&nbsp;&nbsp;<span class="cc">// Same line works for Bus, Train, etc.</span><br>}<br><br><span class="cc">// Person reference — User and Admin interchangeable</span><br><span class="cv">Person</span> p = AuthService.<span class="cm">getLoggedInUser</span>();<br>System.out.<span class="cm">println</span>(p.<span class="cm">getRole</span>()); <span class="cc">// "User" or "Admin"</span></div>
  </div>
  <div style="display:flex;flex-direction:column;gap:12px;justify-content:center;">
    <div class="hl"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g3);margin-bottom:8px;">What LSP Means in SmartGo</div>
    <div class="bullet">Every subclass of Transport <strong>fulfills the full Transport contract</strong> — getPrice(), getCapacity(), getScheduleSummary() — without weakening any guarantee.</div>
    <div class="bullet">BookingService calls flight.getPrice() via the Transport reference. Switching to Bus or Train doesn't change the billing logic.</div>
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;flex:1;">
  <div style="display:flex;flex-direction:column;gap:10px;">
    <div class="card" style="flex:1;"><div style="color:var(--g1);font-weight:700;font-family:Syne,sans-serif;font-size:13px;margin-bottom:8px;">📦 Models</div><div style="font-size:12px;color:var(--muted);line-height:1.6;">Hold data and represent domain objects. No business logic. No file I/O. No UI code.</div><div style="margin-top:8px;font-family:JetBrains Mono,monospace;font-size:10px;color:#475569;">User, Flight, Hotel,<br>Booking, Bill, Payment,<br>Review, TourPlan ...</div></div>
    <div class="card"><div style="color:var(--g1);font-weight:700;font-size:12px;margin-bottom:4px;">Responsibility</div><div style="font-size:11.5px;color:var(--muted);">Getters, setters, toString(). Nothing else.</div></div>
  </div>
  <div style="display:flex;flex-direction:column;gap:10px;">
    <div class="card" style="flex:1;"><div style="color:var(--g2);font-weight:700;font-family:Syne,sans-serif;font-size:13px;margin-bottom:8px;">⚙️ Services</div><div style="font-size:12px;color:var(--muted);line-height:1.6;">Implement business rules. Call DataStore for persistence. Coordinate between models.</div><div style="margin-top:8px;font-family:JetBrains Mono,monospace;font-size:10px;color:#475569;">AuthService,<br>BookingService,<br>FlightService,<br>ReviewService ...</div></div>
    <div class="card"><div style="color:var(--g2);font-weight:700;font-size:12px;margin-bottom:4px;">Responsibility</div><div style="font-size:11.5px;color:var(--muted);">Business logic only. Never touches Swing directly.</div></div>
  </div>
  <div style="display:flex;flex-direction:column;gap:10px;">
...
<div class="two-col">
  <div style="display:flex;flex-direction:column;gap:10px;">
    <div class="code" style="font-size:10.5px;"><span class="cc">// Main.java — captureOutput() helper</span><br><span class="cc">// Services print to System.out. We redirect</span><br><span class="cc">// that output into a String for Swing display.</span><br><br><span class="ck">private static void</span> <span class="cm">captureOutput</span>(<span class="cv">Runnable</span> action, <span class="cs">String</span> title) {<br><br>&nbsp;&nbsp;<span class="cv">ByteArrayOutputStream</span> baos = <span class="ck">new</span> <span class="ct">ByteArrayOutputStream</span>();<br>&nbsp;&nbsp;<span class="cv">PrintStream</span> ps = <span class="ck">new</span> <span class="ct">PrintStream</span>(baos);<br>&nbsp;&nbsp;<span class="cv">PrintStream</span> old = System.out;<br><br>&nbsp;&nbsp;System.<span class="cm">setOut</span>(ps);&nbsp;&nbsp;&nbsp;&nbsp;<span class="cc">// redirect stdout</span><br>&nbsp;&nbsp;action.<span class="cm">run</span>();&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="cc">// service prints here</span><br>&nbsp;&nbsp;System.out.<span class="cm">flush</span>();<br>&nbsp;&nbsp;System.<span class="cm">setOut</span>(old);&nbsp;&nbsp;<span class="cc">// restore stdout</span><br><br>&nbsp;&nbsp;<span class="cv">JTextArea</span> ta = <span class="ck">new</span> <span class="ct">JTextArea</span>(baos.<span class="cm">toString</span>());<br>&nbsp;&nbsp;ta.<span class="cm">setEditable</span>(<span class="ck">false</span>);<br>&nbsp;&nbsp;<span class="cv">JScrollPane</span> sp = <span class="ck">new</span> <span class="ct">JScrollPane</span>(ta);<br>&nbsp;&nbsp;JOptionPane.<span class="cm">showMessageDialog</span>(<span class="ck">null</span>, sp, title, JOptionPane.PLAIN_MESSAGE);<br>}</div>
  </div>
  <div style="display:flex;flex-direction:column;gap:12px;justify-content:center;">
    <div class="hl"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g1);margin-bottom:8px;">Why This Is Architecturally Sound</div>
    <div class="bullet"><strong>Services stay clean</strong> — they print to System.out normally. Zero Swing imports in any service class. UI and logic stay fully separated.</div>
    <div class="bullet"><strong>Runnable functional interface</strong> — Main passes () -&gt; FlightService.showAllFlights() as a lambda. The helper doesn't care what runs inside.</div>
    <div class="bullet">JScrollPane wraps the JTextArea — any amount of output is scrollable, not truncated by a dialog box limit.</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;flex:1;">
  <div class="card" style="display:flex;flex-direction:column;gap:10px;border-top:3px solid var(--g1);"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:15px;color:var(--text);text-transform:uppercase;letter-spacing:0.05em;">Service Layer</div><div style="font-size:12.5px;color:var(--muted);line-height:1.6;">AuthService and BookingService encapsulate all business logic. Main.java coordinates these stateless services without exposing implementation details.</div><div style="font-family:JetBrains Mono,monospace;font-size:10px;color:var(--g1);margin-top:auto;font-weight:700;">IMPLEMENTATION: AuthService.login()</div></div>
  <div class="card" style="display:flex;flex-direction:column;gap:10px;border-top:3px solid var(--g2);"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:15px;color:var(--text);text-transform:uppercase;letter-spacing:0.05em;">Template Method</div><div style="font-size:12.5px;color:var(--muted);line-height:1.6;">Abstract classes Person and Transport define the algorithm skeleton (toString, getPrice) and defer specific steps (getRole, getScheduleSummary) to subclasses.</div><div style="font-family:JetBrains Mono,monospace;font-size:10px;color:var(--g2);margin-top:auto;font-weight:700;">IMPLEMENTATION: Transport.toString()</div></div>
  <div class="card" style="display:flex;flex-direction:column;gap:10px;border-top:3px solid var(--g3);"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:15px;color:var(--text);text-transform:uppercase;letter-spacing:0.05em;">DAO Pattern</div><div style="font-size:12.5px;color:var(--muted);line-height:1.6;">DataStore.java provides an abstract interface (saveUser, loadUsers) that hides all file-based persistence logic from the higher-level service layers.</div><div style="font-family:JetBrains Mono,monospace;font-size:10px;color:var(--g3);margin-top:auto;font-weight:700;">IMPLEMENTATION: DataStore.saveUser()</div></div>
</div>
...
<div class="two-col">
  <div style="display:flex;flex-direction:column;gap:12px;">
    <div class="card"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g1);margin-bottom:8px;">🔐 Password Handling</div><div style="font-size:12.5px;color:var(--muted);line-height:1.6;margin-bottom:8px;">Passwords are stored in <strong style="color:var(--text);">passwordHash</strong> field (named to signal future hashing intent). BCrypt is the noted production upgrade path.</div>
    <div class="code" style="font-size:10px;"><span class="cc">// AuthService.register()</span><br><span class="cc">// Production: BCrypt.hashpw(password, salt)</span><br><span class="cv">User</span> newUser = <span class="ck">new</span> <span class="ct">User</span>(id, name, email, phone, <span class="cy">password</span>, ...);</div></div>
    <div class="card"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g2);margin-bottom:8px;">🛡 Ownership Validation</div><div style="font-size:12.5px;color:var(--muted);line-height:1.6;">Before cancellation or payment, BookingService checks that the booking's userId matches the logged-in user. Users cannot cancel or pay for other users' bookings.</div></div>
  </div>
  <div style="display:flex;flex-direction:column;gap:12px;">
    <div class="card"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g3);margin-bottom:8px;">✅ Input Validation</div>
    <div class="bullet">Email uniqueness enforced at registration — SmartGoException thrown on duplicate.</div>
    <div class="bullet">Review rating validated: must be 1–5. Comment must not be blank.</div>
    <div class="bullet">Payment method limited to enum: CASH, CARD, BANK_TRANSFER — no free-text inputs.</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;flex:1;">
  <div style="display:flex;flex-direction:column;gap:10px;">
    <div class="hl"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g1);margin-bottom:10px;">What We Got Right</div>
    <div class="bullet"><strong>Abstraction pays off</strong> — adding a new booking type (e.g., cruise) requires a new model class + one method in BookingService. Zero existing code changes.</div>
    <div class="bullet"><strong>Single DataStore</strong> was the right call — centralizing all I/O meant we only debugged file parsing once, not six times.</div>
    <div class="bullet"><strong>Custom exceptions</strong> dramatically improved UX — every error is a readable English sentence, not a Java stack trace.</div>
    <div class="bullet"><strong>Enums prevented bugs</strong> — we never had a status-mismatch bug because invalid states simply cannot be compiled.</div></div>
  </div>
  <div style="display:flex;flex-direction:column;gap:10px;">
    <div class="card"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;color:var(--g2);margin-bottom:10px;">What We'd Improve</div>
...
<div style="height:100%;display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;gap:14px;">
  <div>
    ...
    <div style="font-size:24px;color:var(--text);font-weight:500;margin-top:10px;">SmartGo Technical Showcase</div>
    <div style="font-family:'JetBrains Mono',monospace;font-size:13px;color:var(--muted);margin-top:8px;text-transform:uppercase;letter-spacing:0.1em;">End of Presentation</div>
  </div>
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;max-width:900px;width:100%;margin-top:12px;">
    <div class="card" style="text-align:left;"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;">Abdul Rehman</div><div style="font-size:11px;color:var(--muted);margin-top:3px;">Reg. 576841</div></div>
    <div class="card" style="text-align:left;"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;">M. Yousaf Khan</div><div style="font-size:11px;color:var(--muted);margin-top:3px;">Reg. 572229</div></div>
